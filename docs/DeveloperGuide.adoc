...
As sorting is not undoable, this command inherits from the `Command` class instead of the `UndoableCommand` class. The sorting mechanism is mainly facilitated by `LogicManager` and `ModelManager`. `SortCommandParser` residing in `LogicManager` processes user inputs and creates a corresponding `SortCommand`. `ModelManager` will then invoke appropriate sorting methods in `UniquePersonList`.

The following sequence diagram depicts the interactions between `LogicManager` and `ModelManager` when a user issues a request for sorting:

image::
Figure 3.2.1.1 Sequence diagram for execution of `sort` command
...
Currently, HR+ only allows users to sort the list of students by selected fields in ascending or descending order. This is achieved by encapsulating acceptable sorting orders and fields into `SortOrder` and `SortField` respectively as shown below:
[source,java]
----
/**
